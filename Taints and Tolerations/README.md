...
```
The above command will taint the node1 with key "prod", without the appropriate tolerations no pods will be scheduled on node1.
...
Have you noticed pod is in pending state ? Why this is so ? - It's because we have applied taint to node1 but we haven't applied toleration to the nginx pod yet.
